...
clearvars
N=1000; %N is the number of intervals
...
for i=1:N+1
    ...
    if xi(1,i) <= aalpha
        ...
        u(1,i) = aa-aa*(xi(1,i)-aalpha)*(xi(1,i)-ggamma)/((1-aalpha)*(1-ggamma));
    end
    
end

u=c0*ones(1,N+1); %this is the Heaviside initial condition
pu=u;
delu=ones(1,N+1);
u(1,N+1)=0;
pu(1,N+1)=0;
...
b(1,j)=-1.0/dt+lambda*(1.0-2.0*u(1,j))+2*beta/l^2-xi(1,j)*(l-pl)*beta*(hm-hp)/(l*dt);
...
figure
%hold on
plot(dt*(1:maxsteps+1),Lrecord,'g')
hold on
yline(L0+kappa*c0,'--r')
ylim([0 L0])
set(gca, 'XScale', 'log')
